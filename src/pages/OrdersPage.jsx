...
const OrdersPage = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            toast.error('Failed to load orders, error: ' + e);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 py-10">
            <h1 className="text-3xl font-black uppercase tracking-tight text-black mb-10">
                Your Orders
            </h1>

            <div className="space-y-6">
                {orders.map(order => (
                    <Link key={order.id} to={`/orders/${order.id}`} className="border border-gray-200 p-6 block hover:border-black transition-colors">
                        {/* Order header */}
